...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-header-text">Tupoksi List</h5>
                    {{-- button form modal --}}
                        ...
                        <div class="col-md-12">
                            ...
                            <form id="search-form" class="form-inline" role="form">
                                <div class="form-group">
                                    <input type="text" id="tupoksi" name="tupoksi" class="form-control" placeholder="Role Name">
                                </div>
        ...
        {{-- include form modal --}}
        @include('admin.modules.tupoksi.form')
@endsection
...
<script type="text/javascript">
    var oTable = $('#tupoksi-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.api.tupoksi') }}",
                data: function (d) {
                    d.isi_tupoksi = $('input[name=isi_tupoksi]').val();
                }
            },
            columns: [
                {data: 'isi_tupoksi', name: 'isi_tupoksi'},
    ...
    function addForm() {
        ...
        $('.modal-title').text('Tambah Tupoksi');
    }
    function editForm(id) {
        save_method = 'edit';
        $('input[name=_method]').val('PATCH');
        $('#modal-form form')[0].reset();
        $('form[action]').attr("action","{{ url('admin/tupoksi') }}"+"/"+id);
        $.ajax({
          url: "{{ url('admin/api/tupoksi/ajaxtampil') }}" + '/' + id,
          type: "GET",
          // dataType: "JSON",
          success: function(data) {
            $('#modal-form').modal('show');
            $('.modal-title').text('Edit Tupoksi');
            // $('#id').val(data.id);
